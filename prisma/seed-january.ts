...
async function main() {
  console.log("🌱 Seeding January 2026 orders...\n");

  const jan2026Start = new Date("2026-01-01T00:00:00.000Z");
  const jan2026End = new Date("2026-01-31T23:59:59.000Z");

  const seedOrders = Array.from({ length: 20 }, (_, i) => {
    const customer = customers[i % customers.length];
    const product = products[i % products.length];
    const createdAt = randomDate(jan2026Start, jan2026End);
  ...
  console.log(`\n🎉 Done! Seeded ${created} orders for January 2026.`);
}
